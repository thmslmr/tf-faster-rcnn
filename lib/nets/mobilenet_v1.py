...
def separable_conv2d_same(inputs, kernel_size, stride, rate=1, scope=None):
    """
        ...
        Args:
            ...
            scope: Scope.
        Returns:
            output: A 4-D tensor [batch, height_out, width_out, channels] with
...
def mobilenet_v1_base(inputs,
                      ...
                      scope=None):
    """Mobilenet v1.
        Constructs a Mobilenet v1 network from inputs to the given final endpt.
        Args:
            - inputs: a tensor of shape [batch_size, height, width, channels].
            - starting_layer: specifies the current starting layer.
                For region proposal network it is 0, for region classification
                it is 12 by default.
            - min_depth: Minimum depth value (number of channels) for all
                convolution ops. Enforced when depth_multiplier < 1, and not
                an active constraint when depth_multiplier >= 1.
            - depth_multiplier: Float multiplier for the depth
                (number of channels) for all convolution ops. The value must be
                greater than zero. Typical usage will be to set this value in
                (0, 1) to reduce the number of parameters or computation cost
                of the model.
            - conv_defs: A list of ConvDef named tuples specifying
                the net architecture.
            - output_stride: An integer that specifies the requested
                ratio of input to output spatial resolution. If not None, then
                we invoke atrous convolution if necessary to prevent the net
                from reducing the spatial resolution of the activation maps.
            - scope: Optional variable_scope.
        Returns:
            - tensor_out: output tensor corresponding to the final_endpoint.
        Raises:
            - ValueError: if depth_multiplier <= 0, or convolution type is
                not defined.
    """
    def depth(d):
    ...
    with tf.variable_scope(scope, 'MobilenetV1', [inputs], reuse=reuse):
        ...
        # the current network layer. This allows us to invoke atrous conv
        # whenever applying the next convolution would result in the
        # activations having output stride larger than the target output_stride
        current_stride = 1
        ...
        for i, conv_def in enumerate(conv_defs):
            ...
            if isinstance(conv_def, Conv):
                ...
                net = resnet_utils.conv2d_same(net, depth(conv_def.depth),
                                               conv_def.kernel,
    ...
    def fix_variables(self, sess, pretrained_model):
        ...
        with tf.variable_scope('Fix_MobileNet_V1'):
            with tf.device("/cpu:0"):
                ...
                Conv2d_0_rgb = tf.get_variable(
                    "Conv2d_0_rgb",
                    [3, 3, 3, max(int(32 * self._depth_multiplier), 8)],
                    trainable=False)
                restorer_fc = tf.train.Saver(
                    {self._scope + "/Conv2d_0/weights": Conv2d_0_rgb})
                restorer_fc.restore(sess, pretrained_model)

                sess.run(
                    tf.assign(
                        self._variables_to_fix[
                            self._scope + "/Conv2d_0/weights:0"],
