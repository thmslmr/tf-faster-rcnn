...
class Network(object):
    ...
    def _reshape_layer(self, bottom, num_dim, name):
        ...
        with tf.variable_scope(name):
            ...
            reshaped = tf.reshape(to_caffe, tf.concat(
                axis=0, values=[[1, num_dim, -1], [input_shape[2]]]))
    ...
    def _proposal_target_layer(self, rois, roi_scores, name):
        with tf.variable_scope(name):
            (rois,
             ...
             bbox_targets,
             bbox_inside_weights,
             bbox_outside_weights) = tf.py_func(
    ...
    def _anchor_component(self):
        with tf.variable_scope('ANCHOR_' + self._tag):
    ...
    def _smooth_l1_loss(self, bbox_pred, bbox_targets, bbox_inside_weights,
        ...
        in_loss_box = tf.pow(in_box_diff, 2) * (sigma_2 / 2.) * \
    ...
    def _region_proposal(self, net_conv, is_training, initializer):
        ...
        rpn_cls_prob_reshape = self._softmax_layer(
        ...
        if is_training:
            ...
            with tf.control_dependencies([rpn_labels]):
                rois, _ = self._proposal_target_layer(rois,
                                                      roi_scores,
                                                      "rpn_rois")
        else:
    ...
    def create_architecture(self, mode, num_classes, tag=None,
        ...
        # List as many types of layers as possible,
        ...
        if testing:
            stds = np.tile(np.array(
                cfg.TRAIN.BBOX_NORMALIZE_STDS),
                (self._num_classes))

            means = np.tile(np.array(
    ...
    def train_step_with_summary(self, sess, blobs, train_op):
        ...
        (rpn_loss_cls, rpn_loss_box, loss_cls,
         loss_box, loss, summary, _) = sess.run(
            [self._losses["rpn_cross_entropy"],
             self._losses['rpn_loss_box'],
             self._losses['cross_entropy'],
             self._losses['loss_box'],
