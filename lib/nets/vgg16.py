        ...
        with tf.variable_scope('Fix_VGG16'):
            with tf.device("/cpu:0"):
                ...
                restorer_fc = tf.train.Saver(
                    {self._scope + "/fc6/weights": fc6_conv,
                ...
                sess.run(
                    tf.assign(
                        self._variables_to_fix[self._scope + '/fc6/weights:0'],
                        tf.reshape(fc6_conv, self._variables_to_fix[
                            self._scope + '/fc6/weights:0'].get_shape())))

                sess.run(
                    tf.assign(
                        self._variables_to_fix[self._scope + '/fc7/weights:0'],
                ...
                sess.run(
                    tf.assign(
                        self._variables_to_fix[
                            self._scope + '/conv1/conv1_1/weights:0'],
