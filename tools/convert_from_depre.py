...
# --------------------------------------------------------
"""
    Convert depreciated VGG16 snapshots to the ones that support
    tensorflow format.

    It will check the specific snapshot at the vgg16_depre folder,
    and copy it to the same location at vgg16 folder.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Convert an old VGG16 snapshot to new format')
...
def convert_from_depre(net, imdb, input_dir, output_dir, snapshot, max_iters):
    ...
    with sess.graph.as_default():
        ...
        layers = net.create_architecture(sess, 'TRAIN', num_classes,
                                         tag='default',
        ...
        lr = tf.Variable(cfg.TRAIN.LEARNING_RATE * cfg.TRAIN.GAMMA,
                         trainable=False)
        ...
        if cfg.TRAIN.DOUBLE_BIAS:
            ...
            optimizer.apply_gradients(final_gvs)
        else:
        ...
        get_variables_in_checkpoint_file(checkpoint)
...
if __name__ == '__main__':
    ...
    convert_from_depre(net, imdb, input_dir, output_dir,
                       args.snapshot, args.max_iters)
